...
| R38 | **Spot tags (shared fishing markers)** | New `public.spot_tags` Supabase table — player-dropped, time-limited markers shared with friends. Each row TTLs after 24h via `expires_at`; the public-read RLS already filters expired rows. Press **T** during a run to open a label prompt; submitting drops a tag at your current boat position. Friend tags + your own poll alongside presence (24s cadence). Minimap renders yellow flag icons (pole + triangle) on every tag — so when a friend finds a hot spot, you see where they marked it next time you boot up |
| R39 | **What's New panel (feature discovery)** | After ~18 rounds of features, returning players can't use what they don't know exists. A dedicated overlay lists the social/visual highlights (Paint Shop, Winter, Pier Hut, Cloud + Daily Challenge, Friends & Crew) with one-tap launch buttons for each. Auto-pops once for returning players (anyone with achievements / catches / a played run) the first time they land on the marina after R38; persisted dismissal in `tutorialSeen.whatsnew_r38`. A "✨ What's New" button on the s1 marina overlay lets anyone re-open it any time |
| R40 | **Photo mode share/export** | Press **S** during photo mode to download the current scene as a PNG. Captures the live THREE.js canvas (renderer now constructed with `preserveDrawingBuffer:true`), composites a metadata stamp at the bottom (handle, boat, current score, civs saved, biggest catch, date), and triggers a blob-URL download with a timestamped filename. Real shareable artifact for social posts. `DS.exportPhoto()` exposed for QA + manual capture |
| R41 | **Crew Chat (marina message wall)** | New `public.crew_messages` Supabase table — 1-line notes (≤140 chars) with 24h TTL, public read filtered to fresh rows. Post box inside the Friends panel; the marina s1 overlay shows a small "Crew Wall · last 3" card with each note's handle (clickable to open the poster's profile), text, and relative time. Polled every 20s alongside the rest of the social bus. Closes the social loop after R37 presence + R38 spot tags |
| R42 | **Full-screen Lake Atlas** | Press **Tab** to open a full-screen map of the whole lake. 900×700 canvas with: shoreline ring, compass, every named POI, bait shop (diamond + name), shore camp, Pier Hut, marina dock, waiting civilian, active mission, crew presence (green halo + handle), spot tag (yellow flag + label), and your own boat with heading. Legend below. Live-redraws every frame while open so friend positions stay current. Close with Tab, Escape, or click outside |
| R43 | **Atlas waypoints** | Click any marker on the atlas — POI, shop, camp, hut, marina, or active mission — to set a navigation **waypoint**. The minimap shows a small marker ring + a faint bearing line from your boat to the target so the heading reads at a glance. Waypoint renders on the atlas too (color-matched halo + "◎ label" badge). **C** clears it manually; otherwise it auto-clears when you get within 8u of the target and a "ARRIVED" toast fires. `_atlasHits` is rebuilt each render so the click-target list stays in sync with what's actually drawn |
| R44 | **Minimap click → waypoint** | The minimap is always on; clicking it now drops a waypoint at the projected world position (inverse of `drawMinimap`'s `proj` — handles the camX/camZ offset when zoomed in). Click near the center boat dot (within 8px) to clear an active waypoint instead. Fastest UX path from "I want to go there" → "boat starts navigating" — no need to open the atlas |
| R45 | **Player flag emblem** | `profiles.flag` column (≤4 chars, nullable, CHECK enforced). Curated catalog of 10 emoji emblems (🦅🐊🎣⛵🌙⚡🔥💎🌊🪝). Picker on your own profile card (none + 10 options); selection persists in save + syncs to your profile row. A unified `_flagByHandle` lookup is populated by every profile/presence/search fetch; helper `flagFor(handle)` renders the flag wherever a handle is shown — tournament leaderboard rows now lead with the flag, profile avatar tile shows the picked emblem at full size |
| R46 | **Audio polish (winter music + social cues)** | New `winter` entry in `MUSIC_MODES` (D minor wider voicing + cooler filter); `music.update()` auto-picks it when `wx.c==='Snow'`. Four new sfx cues distinct in timbre so they're learnable by ear: **tag** (bright upward chirp — spot tag dropped), **post** (softer two-tone — crew chat posted), **online** (little bell-ping — friend came online, fires once per friend per session), **navset** (downward chirp — waypoint set). Wired at the existing call sites; muted-flag-respecting through the existing `sfx()` map |
| R47 | **Controls reference + accessibility** | The Settings → Controls tab keymap was stale (missing six rounds of new keys). Now lists everything: H (hut), T (spot tag), Tab (atlas), C (clear waypoint), P/S (photo + save), plus the click-to-set-waypoint and click-a-handle mouse actions. Two new accessibility toggles in Settings → Graphics: **Reduced Motion** (zeroes camera shake + suppresses cosmetic droplet particles) and **High-Contrast Markers** (white outlines + larger radius on minimap drop-points + civilians so hue isn't the only cue). Both persisted in `save_v1` |
| R48 | **Activate cloud (env.js) + fix a boot-crash it exposed** | Static `public/env.js` fallback ships the publishable Supabase config (URL + `sb_publishable_…` key) so the entire R22–R46 cloud/social layer goes **live** without Vercel dashboard access. Fills `window.__ENV__` blanks only — real `/api/config` env values always win. **Critically**, turning on `cloudReady()` (whether via this file or by setting the Vercel env vars) exposed a latent temporal-dead-zone crash: `refreshAuthPill()` runs at boot and read `_authPillNote` before its `let` executed — white-screening the game the instant cloud was enabled. Hoisted the declaration to the auth-state block. `DS.cloudReady()` exposed; smoke updated to assert the fallback activates cloud cleanly |
| R48b | **Harden the live cloud layer (review fixes)** | Landed in PR #50's later commits once cloud reads went live, closing what automated review flagged. **Stored-XSS escaping**: every cloud-sourced string (handle, boat name, crew-message text, broadcast `ach_name`, profile flag) now passes through an `esc()` helper before innerHTML, so a crafted handle/message can't execute for other players on a leaderboard, profile, friends list, or crew wall (spot-tag/waypoint labels were already safe — canvas `fillText`). **Stale-save guard**: a silent session restore now runs the same `runPostAuthSync()` as the magic-link callback (pull+merge before any debounced push), so a reload can't clobber the server save. **Two more boot-TDZ hoists** (`_peekGen`, `_peekOpen`) for the invite→`openProfilePanel` path. **Bounded cloud reads**: `withTimeout()` wraps every awaited social-panel fetch so a slow Supabase can't strand a panel on "Loading…" |
| R49 | **Scope the cloud fallback to production** | The `env.js` fallback only fills config when **both** halves are blank (never mixing a configured URL with the fallback key), and it now withholds the fallback on Vercel **preview** hosts — allowlisting the three production domains (`dockshield.vercel.app`, `dockshield-dojoroc.vercel.app`, `dockshield-git-main-dojoroc.vercel.app`) while leaving localhost, the test runner, and any custom domain active. Throwaway branch/per-commit `*.vercel.app` builds now show the offline banner instead of reading/writing the production social tables |
| R58 | **The Drowned Orchard (new fishing spot)** | World expansion on the open east shelf: a new named `FISH_SPOT` ("The Drowned Orchard", x100/z10/r20) that 3×-biases cover/structure fish, a matching `POI` marker (renders on the atlas + minimap), and a new rare species — **Mirror carp** 🎏 (Codex 30 → 31) that holds in the submerged tree rows. A discovery achievement, **Into the Orchard**, fires the first time the boat enters the spot radius (hooked into the existing spot-name display). `DS.qaOrchard()` reports the species/spot/bias/POI/achievement; smoke asserts all of them + species count ≥31 |
| R57 | **Daily first-run bonus** | A retention hook on the existing streak system: the first run of each day pays out a bait bonus — `dailyBonusAmount = 20 + min(streak,10)×3`, so 20 base climbing to a +30 cap at a 10-day streak (35 at the smoke's seeded 5-day streak). Wired into the streak block in `startGame` that already detects a new local day; fires a `DAILY BONUS · +N bait` toast. Pure-function curve so it's testable; `DS.qaDailyBonus(n)` exposes it. Smoke asserts the curve (20/35/50/50) and that the grant actually lands on a fresh-day run |
| R56 | **Night Bite in the What's New tour** | Capstone for the night arc — a `🌙 Night Bite` entry in the `WHATS_NEW` feature tour so players who open the panel learn the whole arc exists (nocturnal tier, Lantern Lure, dark music, the Records night-bite stat). Kept the existing seen-flag (no forced re-pop — the R55 in-run cue is the primary nudge; this is the reference card). Smoke asserts the entry renders alongside the existing ones |
| R55 | **Nightfall cue (night-bite discoverability)** | The R52 nocturnal tier is invisible unless you happen to fish past dusk — so when night falls *during a run*, a one-time `NIGHTFALL · The night bite is on` toast now fires, pointing at the night species + Lantern Lure. Hooked into the existing day/night hysteresis transition; extracted to `announceNightfall()` guarded by a per-run `_nightAnnounced` flag (armed in `startGame` only if the run didn't already start after dark, so it never announces a state you began in). `DS.qaNightfall()` proves it fires once, renders the toast, and that a second call is a no-op |
| R54 | **Pier Hut Records board + jukebox modes** | A stats dashboard in the hut, pulled entirely from already-persisted state (no new save fields): Codex `caught/total`, Achievements `unlocked/total`, Best run score, Biggest catch, **Night bite** `nocturnal caught/total` (surfaces the R52 tier), best Day streak, Duct attempts + near-catches, and Bayou Files lore recovered. Complements the Trophy Wall (species) with the numbers. Also surfaced the two music pads that had no jukebox button — **WINTER** (R46) and **NIGHT** (R53) — so players can actually preview them. Smoke asserts the Records section, three of its stats, and the new jukebox modes render |
| R53 | **Night ambience — `night` music mode** | Completes the Night Bite arc: R52 gave the dark a *reason* (nocturnal tier + Lantern Lure), R53 gives it a *mood*. New `night` entry in `MUSIC_MODES` — a low A-minor drone (55/110/165 Hz), slow LFO, heavily muffled filter — and `music.update()` auto-picks it when `_isNight` and it isn't snowing (precedence: chase > golden > winter > night > explore). Mirrors R46's winter pad exactly; glides in via the existing no-restart `setMode`. `qaAudioProbe()` now reports `musicMode` + `musicModes` so the smoke asserts the mode exists and (best-effort) that it auto-picks after dark |
| R52 | **Night Bite — nocturnal species + Lantern Lure** | Night fishing had only two `night:true` legendaries (0.3% weight), so fishing after dark had no attainable reward. Added **5 nocturnal species** across uncommon/rare (Bullhead, Walleye, Moon perch, Burbot, Lantern-eye gar — Codex 25 → 30) plus a craftable **Lantern Lure** (🏮, crickets + a crayfish). `rollFish`'s night gate is now a tested `nightWeight()` helper: night species roll ×2 after dark, ×0 by day — **unless** the Lantern Lure is equipped, which draws them up by day at ×0.5 (its baitBias then adds ×2.5). Two achievements key off existing state with no new save fields: **Night Owl** (3 nocturnal species in the Codex) and **Lantern Keeper** (land a night fish in daylight — only possible with the lure). `DS.qaNightBite()` exposes the real gate factors + content for the smoke |
| R51 | **Input hygiene for shared cloud text** | `esc()` (R49) neutralises HTML at *render* time, but it doesn't touch control chars, zero-width/joiner chars, or bidi-override chars — which render as invisible garbage or let one handle spoof another (e.g. `admin‮…`) in the now-public shared tables. New `sanitizeText(s,max,singleLine)` strips C0/C1 controls (keeping tab/newline/CR so they collapse to spaces), zero-width (200B–200D), LRM/RLM (200E–200F), bidi embeddings/overrides (202A–202E), word-joiner (2060) and BOM (FEFF); collapses whitespace on single-line fields; trims; hard-caps length. Emoji survive. Wired into every typed-then-stored path: `setHandle`, `setBoatName`, `dropSpotTag`, `postCrewMessage`, plus the save/cloud load paths so a pre-existing dirty save cleans itself. Verified 11/11 unit cases + a smoke assertion that feeds a zero-width/bidi/control-laced handle and asserts the stored value is clean (codepoint-checked) with emoji intact |
| R50 | **Boot-crash regression guard (TDZ class)** | The temporal-dead-zone bug hit three times during cloud activation (`_authPillNote`, `_peekGen`, `_peekOpen`) — each a boot-time read of a `let` declared below the boot call. Audited the whole boot fan-out (`refreshTrophyPeek` → auth/challenge/tournament/friends/crew refreshers + `popInvitePromptIfPending` → `openProfilePanel`) and confirmed no remaining synchronous reads of post-boot state. Then closed the test gap that let all three ship: the smoke never exercised the **signed-in + invite** boot path. New assertion seeds a valid `dockshield_auth_v1` session + an `?invite=` for a different user, then asserts the IIFE completes (`DS` defined) with no "before initialization" pageerror — catching both the synchronous (IIFE-abort) and async (`openProfilePanel` rejected-promise) TDZ shapes. Verified it fails on a re-introduced `_peekOpen` TDZ and passes once restored |
